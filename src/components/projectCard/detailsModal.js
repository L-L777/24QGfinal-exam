...
const DetailModal = ({
  ...
  const checkMyAuth = async () => {
    ...
    try {
      ...
      setBtnLoading(false);
      showModal();
      alertShow(setAlertStatus, {
        type: "error",
        message: "请求错误，请重新点击详情",
        status: true,
      });
    }
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          {role.role === "管理员" && selectedLog === "3" && (
            <Flex style={{ marginTop: "10px", width: "100%" }} vertical>
              <Input.TextArea
                rows={6}
                onChange={rejectValue}
                placeholder="请输入你的拒绝理由：（同意则不用输入）"
              ></Input.TextArea>
              <Flex
