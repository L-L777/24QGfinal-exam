...
import { attackData } from '../../mock/data'
import { queryAttackServerLog } from "../../api"
...
const columns = [
    ...
    },
    {
        ...
        dataIndex: 'illegalRequest',
        render: (text) => <div style={{ wordWrap: 'break-word', whiteSpace: 'normal', fontSize: '16px', textAlign: 'center' }}>{text}</div>,
        width: '60%',
    },
    {
        title: 'Time',
        dataIndex: 'illegalRequestTIme',
        render: (text) => <div style={{ wordWrap: 'break-word', whiteSpace: 'normal', fontSize: '16px', textAlign: 'center' }}>{text}</div>,
        width: '20%',
    },
];
const AttackLog = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData(){
            try {
                const response = await queryAttackServerLog(current, pageSize)
                ...
                setCurrent(1)
                setPageSize(5)
                setTotal(attackData.total)
                setLogData(attackData.data)
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <Flex style={{ width: '90%', marginTop: '30px' }} vertical>
            <h3 style={{ color: "#3F1575", fontSize: '24px' }}>攻击日志</h3>
